...
- Local text files under ``profiling/local/`` (one absolute path per line,
  ``#`` comments allowed):
  ``profile_paths_single.txt``, ``profile_paths_multiple.txt``,
  ``profile_paths_images.txt``, and ``profile_paths_videos.txt``.
...
_PROFILE_LOCAL_DIR = _APP_DIR.parent / "profiling" / "local"


def _paths_from_lines_file(filename: str) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    for path in (_PROFILE_LOCAL_DIR / filename, _APP_DIR / filename):
        if not path.is_file():
            continue
...
def _paths_from_files(*filenames: str) -> list[str]:
    return _merge_paths(*(_paths_from_lines_file(filename) for filename in filenames))
...
def _profile_duration_ms(env_key: str, default_ms: int) -> int:
    try:
        return max(500, int(os.environ.get(env_key, str(default_ms))))
    except ValueError:
        logging.warning("Invalid %s value, using %s ms", env_key, default_ms)
        return default_ms


def _first_configured_path(env_key: str, *files: str) -> str:
    paths = _merge_paths(_paths_from_env(env_key), *(_paths_from_lines_file(f) for f in files))
    return paths[0] if paths else ""


def _image_files_in_folder(folder: str, limit: int = 5) -> list[str]:
    try:
        names = sorted(os.listdir(folder))
    except OSError as exc:
        logging.error("Cannot read image profile folder %s: %s", folder, exc)
        return []

    images: list[str] = []
    for name in names:
        path = os.path.join(folder, name)
        if os.path.isfile(path) and path.lower().endswith(IMAGE_FORMATS):
            images.append(path)
            if len(images) >= limit:
                break
    return images
...
def _close_profile_app(app) -> None:
    """Close Tk and stop executors so cProfile batch processes can exit."""
    for attr in ("executor", "io_executor"):
        executor = getattr(app, attr, None)
        if executor is None:
            continue
        try:
            executor.shutdown(wait=False, cancel_futures=True)
        except Exception:
            logging.debug("Failed to shut down %s during profiling close", attr, exc_info=True)
...
def _ensure_timeline_visible(app) -> bool:
    """Show the timeline panel for timeline profiles without saving user preferences."""
    container = getattr(app, "timeline_container", None)
    widget = getattr(app, "timeline_widget", None)
    if container is None or widget is None:
        logging.warning("[PROFILE] Timeline panel/widget is not available.")
        return False

    try:
        if not getattr(container, "expanded", False):
            app.toggle_timeline_menu(save_prefs=False)
        else:
            app.ShowTWidget = True
            show_var = getattr(app, "show_timeline_var", None)
            if show_var is not None:
                show_var.set(True)
        app.update_idletasks()
    except Exception:
        logging.exception("[PROFILE] Failed to show timeline panel")
        return False

    try:
        visible = bool(widget.winfo_exists() and widget.winfo_viewable())
    except Exception:
        visible = False
    logging.info(
        "[PROFILE] Timeline enabled: ShowTWidget=%s visible=%s",
        getattr(app, "ShowTWidget", None),
        visible,
    )
    return visible
...
def profile_grid_render(log_path, workers=None, batch=None):
    ...
    folders_to_profile = _merge_paths(
        ...
        _paths_from_files("profile_paths_single.txt", "profile_paths_grid.txt"),
    )
    if not folders_to_profile:
        logging.warning(
            "No grid profile folders: set VIBE_PLAYER_PROFILE_GRID_FOLDERS (%s-separated) "
            "or create profiling/local/profile_paths_single.txt (one folder per line).",
            os.pathsep,
        )
        app.after(1000, lambda: _close_profile_app(app))
        return

    def run_sequence(index):
        if index >= len(folders_to_profile):
            logging.info("Grid stress test completed, closing.")
            app.after(5000, lambda: _close_profile_app(app))
            return
...
def profile_folder_tree(log_path, workers=None, batch=None):
    """Profile initial folder-tree population and keep the UI alive briefly."""
    duration_ms = _profile_duration_ms("VIBE_PLAYER_PROFILE_TREE_MS", 5000)
    app = VideoThumbnailPlayer(log_path=log_path)
    apply_overrides(app, workers, batch)

    def run_tree_profile():
        start = time.perf_counter()
        try:
            app.populate_tree()
        except Exception:
            logging.exception("[TREE PROFILE] populate_tree failed")
            _close_profile_app(app)
            return
        elapsed = (time.perf_counter() - start) * 1000
        logging.info("[TREE PROFILE] populate_tree returned in %.1f ms", elapsed)
        app.after(duration_ms, lambda: _close_profile_app(app))

    app.after(500, run_tree_profile)
    app.mainloop()


def profile_tree_stress(log_path, workers=None, batch=None):
    """Repeatedly open top-level tree nodes to catch slow tree expansion paths."""
    app = VideoThumbnailPlayer(log_path=log_path)
    apply_overrides(app, workers, batch)
    max_nodes = int(os.environ.get("VIBE_PLAYER_PROFILE_TREE_STRESS_NODES", "12"))
    dwell_ms = _profile_duration_ms("VIBE_PLAYER_PROFILE_TREE_STRESS_DWELL_MS", 500)
    ...
    def start():
        try:
            ...
            logging.exception("[TREE STRESS] populate_tree failed")
            _close_profile_app(app)
            return

        nodes = list(app.tree.get_children(""))[:max_nodes]
        if not nodes:
            logging.error("[TREE STRESS] no root tree nodes available")
            _close_profile_app(app)
            return
        run_step(nodes, 0)

    def run_step(nodes, index):
        if index >= len(nodes):
            if timings:
                avg = sum(t for _, t in timings) / len(timings)
                logging.info("[TREE STRESS] AVG expand time: %.1f ms", avg)
            app.after(1000, lambda: _close_profile_app(app))
            return

        node = nodes[index]
        label = app.tree.item(node, "text")
        start_time = time.perf_counter()
        try:
            app.tree.focus(node)
            app.tree.selection_set(node)
            app.tree.item(node, open=True)
            app.open_node(type("TreeEvent", (), {"y": None})())
            elapsed = (time.perf_counter() - start_time) * 1000
            timings.append((label, elapsed))
            logging.info("[TREE STRESS] %s expanded in %.1f ms", label, elapsed)
        except Exception:
            logging.exception("[TREE STRESS] failed on node %s", label)

        app.after(dwell_ms, lambda: run_step(nodes, index + 1))

    app.after(500, start)
    app.mainloop()


def profile_visible_thumbnails(log_path, workers=None, batch=None):
    """Profile the first visible thumbnail load window for one configured folder."""
    target_folder = _first_configured_path(
        "VIBE_PLAYER_PROFILE_VISIBLE_FOLDERS",
        "profile_paths_single.txt",
        "profile_paths_visible.txt",
        "profile_paths_grid.txt",
    )
    if not target_folder:
        logging.error(
            "No visible-thumbnail folder: set VIBE_PLAYER_PROFILE_VISIBLE_FOLDERS "
            "or create profiling/local/profile_paths_single.txt."
        )
        return
    if not check_path(target_folder):
        return

    measure_ms = _profile_duration_ms("VIBE_PLAYER_PROFILE_VISIBLE_MS", 6000)
    app = VideoThumbnailPlayer(log_path=log_path)
    apply_overrides(app, workers, batch)

    def start():
        logging.info("[VISIBLE PROFILE] folder=%s window=%s ms", target_folder, measure_ms)
        app.display_thumbnails(target_folder)
        app.after(measure_ms, lambda: _close_profile_app(app))
...
def profile_timeline_thumbnails(log_path, workers=None, batch=None):
    ...
    """
    VIDEO_DIR = _first_configured_path(
        "VIBE_PLAYER_PROFILE_VIDEO_DIR",
        "profile_paths_videos.txt",
        "profile_video_dir.txt",
        "profile_paths_single.txt",
        "profile_paths_grid.txt",
    )
    NUM_THUMBS = 5
    ...
    def start_test():
        ...
        if not check_path(VIDEO_DIR):
            ...
            _close_profile_app(app)
            return
        if not _ensure_timeline_visible(app):
            _close_profile_app(app)
            return
    ...
    def run_step(idx, test_videos):
        if idx >= len(test_videos):
            print_summary()
            app.after(1500, lambda: _close_profile_app(app))
            return
    ...
    def run_timeline_sequence():
        ...
        def open_step(idx):
            if idx >= len(test_videos):
                ...
                logging.info(f"{'='*55}\n")
                app.after(2000, lambda: _close_profile_app(app))
                return
...
def profile_wide_folders(log_path, workers=None, batch=None):
    ...
    if not valid_folders:
        logging.error(
            ...
            "create profiling/local/profile_paths_single.txt."
        )
        _close_profile_app(app)
        return
...
def profile_folder_switching(log_path, workers=None, batch=None):
    ...
    folders_to_test = _merge_paths(
        ...
        _paths_from_files("profile_paths_multiple.txt", "profile_paths_switching.txt"),
    )
    if not folders_to_test:
        logging.warning(
            "No folders for switching test. Set VIBE_PLAYER_PROFILE_SWITCH_FOLDERS or "
            "profiling/local/profile_paths_multiple.txt."
        )
        app.after(1000, lambda: _close_profile_app(app))
        return
...
def profile_selection_stress(log_path, workers=None, batch=None):
    ...
    """
    target_dir = _first_configured_path(
        "VIBE_PLAYER_PROFILE_SELECTION_DIR",
        "profile_paths_single.txt",
    ...
    def start_test():
        if not target_dir:
            logging.error(
                ...
                "profiling/local/profile_paths_single.txt"
            )
            _close_profile_app(app)
            return
...
def profile_image_viewer(log_path, workers=None, batch=None):
    """
    Profile image viewer open/render/zoom/pan operations for a small image sample.

    Configure the folder with ``VIBE_PLAYER_PROFILE_IMAGE_DIR`` or
    ``profile_image_dir.txt``. Falls back to ``profile_paths_grid.txt``.
    """
    image_dir = _first_configured_path(
        "VIBE_PLAYER_PROFILE_IMAGE_DIR",
        "profile_paths_images.txt",
        "profile_image_dir.txt",
        "profile_paths_single.txt",
        "profile_paths_grid.txt",
    )
    if not image_dir:
        logging.error(
            "No image folder: set VIBE_PLAYER_PROFILE_IMAGE_DIR or create "
            "profiling/local/profile_paths_images.txt."
        )
        return
    if not check_path(image_dir):
        return

    images = _image_files_in_folder(image_dir, limit=5)
    if not images:
    ...
    app = VideoThumbnailPlayer(log_path=log_path)
    apply_overrides(app, workers, batch)
    app.video_files = [{"path": p, "name": os.path.basename(p), "is_folder": False} for p in images]
    timings: list[tuple[str, str, float]] = []

    def timed(name, label, func):
        start = time.perf_counter()
        result = func()
        elapsed = (time.perf_counter() - start) * 1000
        logging.info("[IMAGEVIEW] %s %s: %.1f ms", name, label, elapsed)
    ...
    def run():
        ...
        timed(first_name, "open_image_viewer", lambda: app.open_image_viewer(first, first_name))
        viewer = getattr(app, "current_image_window", None)
        if viewer is None:
            logging.error("[IMAGEVIEW] viewer did not open")
            _close_profile_app(app)
            return
        ...
        for path in images[1:]:
            ...
            timed(current_name, "load_image", lambda p=path, n=current_name: viewer.load_image(p, n))

        for zoom_factor in (0.5, 1.0, 2.0, 4.0):
            if hasattr(viewer, "zoom_factor"):
                viewer.zoom_factor = zoom_factor
            timed(current_name, f"update_image LQ {zoom_factor:.1f}x", lambda: viewer.update_image(False))
            timed(current_name, f"update_image HQ {zoom_factor:.1f}x", lambda: viewer.update_image(True))

        if hasattr(viewer, "do_pan"):
            class PanEvent:
                x = 80
                y = 80
            ...
            for step in range(5):
                PanEvent.x += 20
                PanEvent.y += 15
                timed(current_name, f"pan step {step + 1}", lambda: viewer.do_pan(PanEvent))

        if timings:
            avg = sum(t for _, _, t in timings) / len(timings)
            logging.info("[IMAGEVIEW] AVG measured operation: %.1f ms", avg)
        app.after(1000, lambda: _close_profile_app(app))
...
def launch_only(log_path, workers=None, batch=None):
    """Measure startup without letting delayed last-folder restore start thumbnail workers."""
    app = VideoThumbnailPlayer(log_path=log_path)
    apply_overrides(app, workers, batch)
    app.after(300, lambda: _close_profile_app(app))
    app.mainloop()
...
def profile_cprofile_grid(log_path, workers=None, batch=None):
    ...
    if not folders:
        logging.error(
            ...
            "  Create profiling/local/profile_paths_single.txt with one absolute folder path per line,\n"
...
def profile_cprofile_switching(log_path, workers=None, batch=None):
    ...
    if not folders:
        logging.error(
            ...
            "  Create profiling/local/profile_paths_multiple.txt (one absolute path per line)\n"
...
def main():
    ...
    profiles = {
        ...
        '--visiblethumbnails': profile_visible_thumbnails,
        '--imageviewer':    profile_image_viewer,
        '--treestress':     profile_tree_stress,
